...
export const logHabit = catchAsync(async (req, res, next) => {
  ...
  if (!habit.days.includes(dayOfWeek)) {
    return next(new AppError("This date is not scheduled for this habit", 400));
  }

  // Check progress doesn't exceed goal
  if (progress_value && progress_value > habit.goal_target) {
    return next(
      new AppError(`Progress cannot exceed goal (${habit.goal_target})`, 400),
    );
  }
  ...
  recalculateStreaks(habitId, userId, timezone).catch((err) =>
    console.error("Streak calc failed:", err),
  );
